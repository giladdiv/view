...
def view_layers(x, input_shape, trainable=False):
    ...
    iner_layer = identity_block_td(x, 3, [512, 512, 2048], stage='_view_', block='c', trainable=trainable)
    x = TimeDistributed(AveragePooling2D((7, 7)), name='avg_pool_view')(iner_layer)
...
def classifier_with_iner(base_layers, input_rois, num_rois, nb_classes = 21, trainable_cls= False ,trainable_view = False):
    ...
    out1,iner_layer = view_layers(out_roi_pool, input_shape=input_shape, trainable=trainable_view)
...
def classifier(base_layers, input_rois, num_rois, nb_classes = 21, trainable_cls= False ,trainable_view = False):
